        ...
        <div class="management">
            <div class="row">
                        ...
                        <tbody>
                            <tr v-for="(category_parent, index) in category_parents" :key="category_parent.id" 
                              :category_parent="category_parent" :index="index" @click="showChildCategory(category_parent)" 
                                ...
                                <td class="col1">{{category_parent.id}}</td>
                                <td id="col2">
                                  ...
                                  <button type="button" class="btn btn-danger btn-deleteCategory" data-toggle="modal" data-target="#categoryparent">削除</button>
                                      ...
                                      <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                          ...
                                          <div class="modal-footer">
                                              <button type="button" class="btn btn-secondary mr-auto" @click="removeParentCategory(category_parent)">はい</button>
                ...
                <div class="category-child category">
                    <p class="category-child">親カテゴリ１</p>
                    <table class="table table-bordered">
                        <thead>
                        </thead>
                        <tbody>
                            <tr v-for="(category_child, index) in category_children" :key="category_child.id" 
                              :category_child="category_child" :index="index" >
                                ...
                                <td id="col2">
                                    ...
                                    <button type="button" class="btn btn-danger btn-deleteCategory" data-toggle="modal" data-target="#categorychild">削除</button>
                                     <!-- Modal confrim delete popup -->
                                    <div class="modal fade" id="categorychild" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary mr-auto" @click="removeChildCategory(category_child)">はい</button>
                                                <button type="button" class="btn btn-primary mr-auto" data-dismiss="modal">いいえ</button>
                                            </div>
                                            </div>
                                        </div>
...
export default {
  ...
  methods: {
    removeParentCategory(category_parent){
      alert(category_parent.id)
      // axios.delete('v1/admin/category/parents/' + category_parent.id)
      // .then(response => {
      //   this.category_parents.splice(this.category_parents.indexOf(category_parent), 1)
      //   window.location.reload()
      // }).catch((e) => {
      // console.log('Loi xoa')
      // })
    },
    removeChildCategory(category_child){
      alert(category_child.id)
      // axios.delete('v1/admin/category/children/' + category_child.id)
      // .then(response => {
      //   this.category_children.splice(this.category_children.indexOf(category_child), 1)
      //   window.location.reload()
      // }).catch((e) => {
      // console.log('Loi xoa')
      // })
